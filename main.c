	...
	// Т.к. в задании не указаны подробности CRC контроля, предположил, что это будет
	// CRC-8, CRC байт будет передаваться в конце сообщения, делитель 0х31
		...
		if (USART1->ISR & USART_ISR_RXNE)    {            				 //если есть принятый байт
			if (N_byte == 0) {
				...
				if (N_byte == (4+3+DATA[4]+1))  {                       //если все байты приняты (счетчик достиг КОЛ-ВО_БАЙТ + 7 + 1), возвращаем счетчик байт в позицию 0, считываем контрольный байт, запускаем проверку CRC
					N_byte = 0;
					TIM14->EGR |= TIM_EGR_UG;                 //прием завершен, останавливаем таймер отсчета 1000мс, сбрасываем регистр счета
